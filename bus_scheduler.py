...
def _select_bus(buses, trip, config, min_break, natural_gap=None):
    ...
    for bus in buses:
        ...
        # Phase re-sync: after charging or dead run, ensure departure is at least
        # natural_gap after the most-recent same-direction bus in EITHER direction.
        # This prevents all returning buses from departing back-to-back.
        if natural_gap:
            last_any = _last_revenue_any_direction(buses, trip.start_location)
            if last_any and last_any.actual_departure:
                min_rt = last_any.actual_departure + timedelta(minutes=natural_gap / 2)
                if rt < min_rt:
                    rt = min_rt
