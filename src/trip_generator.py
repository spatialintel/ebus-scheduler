...
def _generate_revenue_trips(config, headway_df, travel_time_df):
    ...
    end_loc     = config.end_point     # ADHEWADA GAM (nearest node — buses start here)
    ...
    # ── DN thread: end_point → start_point, starts at op_start ──────────────
    dn_time = op_start
    while dn_time < op_end:
        ...
        trips.append(Trip(
            ...
            start_location=end_loc, end_location=start_loc,
        ...
        dn_time += timedelta(minutes=_get_headway_at(dn_time, headway_df))

    # ── UP thread: start_point → end_point ───────────────────────────────────
    # First UP starts after a bus has completed the first DN and taken its break.
    # This is op_start + dn_travel(op_start) + min_break.
    ...
    while up_time < op_end:
        ...
        trips.append(Trip(
            ...
            latest_departure=op_end,
            travel_time_min=up_travel, distance_km=s2e_dist,
            handover=(up_time < shift_split <= up_arrival),
            shift=up_shift,
        ))
        up_time += timedelta(minutes=_get_headway_at(up_time, headway_df))
